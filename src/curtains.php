<?php include_once('partials/header.php'); ?>

<main id="product">
  <section id="hero" class="section">
    <div class="hero">
      <div class="container">
        <div class="hero__background">
          <img src="img/page/curtains/hero/banner.jpg" alt="" class="hero__image image image--cover">
        </div>
        <!-- /.hero__background -->

        <div class="hero__content">
          <div class="section__header">
            <h1 class="hero__title section__title page__title">Рулонные шторы</h1>
            <nav class="nav nav--section">
              <ul class="nav__list">
                <li class="nav__item">
                  <a class="nav__link link link--nav link--arrow" href="#calculator">
                    Перейти к калькулятору
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </a>
                </li>
              </ul>
            </nav>
            <!-- /.nav -->
          </div>
          <!-- /.section__header -->

          <p class="hero__description section__description">
            Наша компания изготавливает москитные сетки и рулонные шторы уже больше 10 лет. Каждая сетка или штора делается строго индивидуально под Ваши окна. В нашем каталоге тысячи цветов и вариаций рисунков для штор, а также, все возможные виды антимоскитного полотна для любой формы москитной сетки.
          </p>
        </div>
        <!-- /.hero__content -->
      </div>
      <!-- /.container -->
    </div>
    <!-- /.hero -->
  </section>

  <?php include_once('partials/calculator.php'); ?>

  <section id="constuction" class="section">
    <div class="container">
      <div class="constuction">
        <h2 class="section__title">Конструкция изделий</h2>
        <p class="section__description">
          Наша компания изготавливает москитные сетки и рулонные шторы уже больше 10 лет. Каждая сетка или штора делается строго индивидуально под Ваши окна. В нашем каталоге тысячи цветов и вариаций рисунков для штор, а также, все возможные виды антимоскитного полотна для любой формы москитной сетки.
        </p>

        <div data-aos="fade-up" class="constuction__slider position-relative">
          <h3 class="section__subtitle">Виды и формы штор</h3>

          <div class="swiper swiper--shadow slider-constuction">
            <div class="swiper-wrapper">
              <div class="swiper-slide">
                <div class="constuction__item">
                  <div class="constuction__picture">
                    <img src="img/catalog/mosquito/constuction/standart.svg" alt="" class="constuction__image image">
                  </div>
                  <!-- /.constuction__picture -->

                  <div class="constuction__content">
                    <div class="constuction__header">
                      <h3 class="constuction__name"><a href="" class="link link--nav">Рамочная сетка</a></h3>
                      <a class="constuction__lightbox" data-fslightbox="id-1" href="img/catalog/mosquito/constuction/standart.svg">
                        <img src="img/icons/misc/help.svg" alt="" class="constuction__help">
                      </a>
                    </div>
                    <!-- /.construction__header -->

                    <span class="constuction__price">от 1 000р</span>
                    <p class="constuction__description">Отличаются простотой в установке. комплектация полотном антипыль и антикошка. Возможна комплектация полотном антипыль и антикошка.</p>

                    <div class="constuction__action">
                      <a href="" class="constuction__link link link--nav link--arrow">
                        Подробнее
                        <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                        </svg>
                      </a>
                    </div>
                    <!-- /.constuction__action -->
                  </div>
                  <!-- /.constuction__content -->
                </div>
                <!-- /.constuction__item -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide">
                <div class="constuction__item">
                  <div class="constuction__picture">
                    <img src="img/catalog/mosquito/constuction/inner.svg" alt="" class="constuction__image image">
                  </div>
                  <!-- /.constuction__picture -->

                  <div class="constuction__content">
                    <div class="constuction__header">
                      <h3 class="constuction__name"><a href="" class="link link--nav">Рамочная сетка</a></h3>
                      <a class="constuction__lightbox" data-fslightbox="id-1" href="img/catalog/mosquito/constuction/inner.svg">
                        <img src="img/icons/misc/help.svg" alt="" class="constuction__help">
                      </a>
                    </div>
                    <!-- /.construction__header -->

                    <span class="constuction__price">от 1 300р</span>
                    <p class="constuction__description">Отличаются простотой в установке, уходе и низкой ценой. Возможна комплектация полотном антипыль и антикошка. Возможна комплектация полотном антипыль и антикошка.</p>

                    <div class="constuction__action">
                      <a href="" class="constuction__link link link--nav link--arrow">
                        Подробнее
                        <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                        </svg>
                      </a>
                    </div>
                    <!-- /.constuction__action -->
                  </div>
                  <!-- /.constuction__content -->
                </div>
                <!-- /.constuction__item -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide">
                <div class="constuction__item">
                  <div class="constuction__picture">
                    <img src="img/catalog/mosquito/constuction/inner.svg" alt="" class="constuction__image image">
                  </div>
                  <!-- /.constuction__picture -->

                  <div class="constuction__content">
                    <div class="constuction__header">
                      <h3 class="constuction__name"><a href="" class="link link--nav">Рамочная сетка</a></h3>
                      <a class="constuction__lightbox" data-fslightbox="id-1" href="img/catalog/mosquito/constuction/provedal.svg">
                        <img src="img/icons/misc/help.svg" alt="" class="constuction__help">
                      </a>
                    </div>
                    <!-- /.construction__header -->

                    <span class="constuction__price">от 1 300р</span>
                    <p class="constuction__description">Далеко-далеко за словесными горами в стране гласных и согласных живут рыбные тексты. Букв ручеек несколько, заманивший большого курсивных, маленькая коварный домах своего океана но все точках всемогущая? Правилами что путь вскоре знаках.</p>

                    <div class="constuction__action">
                      <a href="" class="constuction__link link link--nav link--arrow">
                        Подробнее
                        <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                        </svg>
                      </a>
                    </div>
                    <!-- /.constuction__action -->
                  </div>
                  <!-- /.constuction__content -->
                </div>
                <!-- /.constuction__item -->
              </div>
              <!-- /.swiper-slide -->
            </div>
            <!-- /.swiper-wrapper -->
          </div>
          <!-- /.swiper constuction-slider -->

          <div class="swiper__control swiper__control--heading">
            <div class="swiper-button-prev slider-constuction__prev"></div>
            <div class="swiper-button-next slider-constuction__next"></div>
          </div>
          <!-- /.swiper__control -->
        </div>
        <!-- /.constuction__slider -->

        <div data-aos="fade-up" class="constuction__slider position-relative mt-5 mt-md-4">
          <div class="section__header mb-0">
            <h3 class="section__subtitle">Светопропускаемость ткани</h3>
            <a href="" class="link link--nav link--arrow">
              Подробнее
              <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
              </svg>
            </a>
          </div>
          <div class="swiper swiper--shadow swiper--mobile slider-illumination">
            <div class="swiper-wrapper row">
              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/85.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/70.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/55.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/35.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/15.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->

              <div class="swiper-slide col-12 col-md-2">
                <div class="illumination mb-4">
                  <div class="illumination__picture">
                    <img src="img/catalog/curtains/illumination/0.svg" alt="" class="illumination__image image image--cover">
                  </div>
                  <!-- /.illumination__picture -->
                </div>
                <!-- /.illumination -->
              </div>
              <!-- /.swiper-slide -->
            </div>
            <!-- /.swiper-wrapper -->
          </div>
          <!-- /.swiper swiper--overflow slider-useful -->
        </div>
        <!-- /.constuction__slider -->
      </div>
      <!-- /.constuction -->
    </div>
    <!-- /.container -->
  </section>

  <section id="catalog" class="section">
    <div class="container">
      <div class="catalog">
        <div class="row align-items-center">
          <div class="col-12 col-md-6">
            <div class="catalog__picture position-relative">
              <img src="img/misc/preloader.svg" class="preloader" alt="">
              <img data-src="img/page/curtains/catalog.jpg" alt="" class="catalog__image image image--cover lazy">
            </div>
            <!-- /.catalog__picture -->
          </div>
          <!-- /.col-6 -->

          <div class="col-12 col-md-5">
            <h2 class="catalog__title">Отправьте заявку на вызов <br> дизайнера с каталогами тканей</h2>
            <p class="catalog__description">Наш дизайнер приедет к Вам с каталогами, расскажет и поможет подобрать лучшее решение для вашего интерьера. Также, проведет замер.</p>
            <button class="button button-primary">Вызвать</button>
          </div>
          <!-- /.col-6 -->
        </div>
        <!-- /.row -->
      </div>
      <!-- /.catalog -->
    </div>
    <!-- /.container -->
  </section>

  <?php include_once('partials/textblock.php'); ?>

  <section id="usage" class="section">
    <div class="container">
      <div class="usage">
        <h2 class="section__title">Варианты установки</h2>
        <div class="row">
          <div data-aos="fade-up" data-aos-delay="100" class="col-12 col-sm-6 col-lg-3 mb-4">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/1.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На пластиковые окна</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="200" class="col-12 col-sm-6 col-lg-3 mb-4">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/2.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На алюминиевые окна</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="300" class="col-12 col-sm-6 col-lg-3 mb-4">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/3.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На деревянные окна</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="400" class="col-12 col-sm-6 col-lg-3 mb-4">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/4.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На форточку</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="500" class="col-12 col-sm-6 col-lg-3 mb-4 mb-lg-0">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/5.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На лоджию</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="600" class="col-12 col-sm-6 col-lg-3 mb-4 mb-lg-0">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/6.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На балкон</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="700" class="col-12 col-sm-6 col-lg-3 mb-4 mb-lg-0">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/7.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На беседку</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->

          <div data-aos="fade-up" data-aos-delay="800" class="col-12 col-sm-6 col-lg-3">
            <div class="card">
              <div class="card__picture position-relative">
                <img src="img/misc/preloader.svg" class="preloader" alt="">
                <img data-src="img/page/mosquito/usage/8.jpg" alt="" class="card__image image image--cover lazy">
              </div>
              <!-- /.service__picture -->

              <div class="card__content">
                <h3 class="card__title">На веранду</h3>
                <span class="card__link link link--arrow">Подробнее
                  <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                  </svg>
                </span>
              </div>
              <!-- /.card__content -->

              <a class="stretched-link" href=""></a>
            </div>
            <!-- /.card -->
          </div>
          <!-- /.col-3 -->
        </div>
        <!-- /.row -->
      </div>
      <!-- /.usage -->
    </div>
    <!-- /.container -->
  </section>

  <section class="section" id="service">
    <div class="container">
      <div class="service">
        <div class="section__header">
          <h2 class="section__title">Другие услуги</h2>
          <a href="" class="link link--nav link--arrow">
            Все услуги
            <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
            </svg>
          </a>
        </div>
        <!-- /.section__header -->

        <p class="section__description">
          Наша компания не только изготавливает москитные сетки и рулонные шторы, но также оказывает различные услуги по обслуживанию, ремонту окон и изделий. Устанавливаем системы защиты для безопасности детей и животных.
        </p>

        <div class="swiper swiper--shadow swiper--mobile slider-service">
          <div class="swiper-wrapper row">
            <div class="swiper-slide col-12 col-sm-6 col-lg-4 mb-3 mb-sm-5">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/adjustment.jpg" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Регулировка окон</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->

            <div class="swiper-slide col-12 col-sm-6 col-lg-4 mb-3 mb-sm-5">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/lock.jpg" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Замки от детей</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->

            <div class="swiper-slide col-12 col-sm-6 col-lg-4 mb-3 mb-sm-5">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/limiter.jpg" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Ограничители (гребенки)</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->

            <div class="swiper-slide col-12 col-sm-6 col-lg-4 mb-3 mb-sm-0">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/tinting.jpg" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Тонировка окон</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->

            <div class="swiper-slide col-12 col-sm-6 col-lg-4 mb-3 mb-sm-0">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/compressor.png" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Замена уплотнителя</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->

            <div class="swiper-slide col-12 col-sm-6 col-lg-4">
              <div class="card">
                <div class="card__picture position-relative">
                  <img src="img/misc/preloader.svg" class="preloader" alt="">
                  <img data-src="img/section/catalog/limiter.jpg" alt="" class="card__image image image--cover lazy">
                </div>
                <!-- /.card__picture -->

                <div class="card__content">
                  <h3 class="card__title">Ограничители (гребенки)</h3>
                  <span class="card__link link link--arrow">Подробнее
                    <svg class="link__arrow svg ms-2" width="13" height="10" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.83007 0.139209L9.86053 3.16392C10.0465 3.34953 10.0465 3.65047 9.86053 3.83608L6.83007 6.86079C6.64411 7.0464 6.3426 7.0464 6.15663 6.86079C5.97067 6.67518 5.97067 6.37424 6.15663 6.18863L8.37418 3.97529L0 3.97529L0 3.02471L8.37418 3.02471L6.15663 0.811367C5.97067 0.625755 5.97067 0.32482 6.15663 0.139209C6.3426 -0.0464029 6.64411 -0.0464029 6.83007 0.139209Z" fill="#969696"></path>
                    </svg>
                  </span>
                </div>
                <!-- /.card__content -->

                <a class="stretched-link" href=""></a>
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col-4 -->
          </div>
          <!-- /.row -->
        </div>
        <!-- /.swiper swiper--shadow swiper--mobile -->
      </div>
      <!-- /.callback -->
    </div>
    <!-- /.container -->
  </section>

  <?php include_once('partials/order.php'); ?>

  <?php include_once('partials/faq.php'); ?>

  <?php include_once('partials/callback.php'); ?>
</main>

<?php include_once('partials/footer.php'); ?>